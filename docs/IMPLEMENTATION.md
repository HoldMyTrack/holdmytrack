...
CREATE TABLE activities (
    ...
    source            VARCHAR(32)  NOT NULL,   -- 'upload' | 'takeout' | 'garmin' | 'wahoo'
                                               -- | 'coros' | 'oura' | 'healthkit' | 'health_connect'
...
* **Android/Health Connect** is constrained in two ways verified against platform and vendor documentation: `READ_EXERCISE_ROUTES` cannot be requested programmatically (the user grants it manually in Health Connect settings), and **routes written by other apps cannot be read in the background** — `ExerciseRouteResult.ConsentRequired` is returned even with "Always allow". Android sync is therefore **foreground-only**, and for **Samsung Health specifically, route geometry is not exposed at all**, so Galaxy Watch sync yields summary metrics and no map.
...
**Built, with concrete adopted values, recorded the same way §4.2.1 recorded fog's.** The accumulator isn't "all tracks share one canvas" (that would just reproduce fog's max, per the reasoning above) — each track renders to its own temporary stroke mask (same stroke radius and feather as fog, so the two mechanics agree on "how wide is a route") and the masks sum into a shared float accumulator, pixel by pixel. The sum is normalized against a fixed global cap of **8.0** accumulated full-strength passes (a first guess — "about 8 separate activities through the same spot reads as maximally hot" — explicitly open to retuning against real usage, not derived from a reference the way fog's numbers were), then compressed with **sqrt**, not log, so a single pass already reads as visibly warm rather than needing to approach the cap to show up. Colour ramp, linearly interpolated between stops: transparent at zero → RGB(178, 24, 24) at 15% (alpha 130) → RGB(255, 140, 0) at 55% (alpha 210) → RGB(255, 235, 60) at 100% (alpha 255).
...
#### 4.2.3 Per-activity masks: how fog/heatmap answer a date-range/hidden-activity filter

Fog and heatmap must respect the selected date range and the eye-icon hidden set — an activity outside the range, or hidden, must not count toward coverage/heat.

**Why this isn't a live query-parameter filter, the way §4.3's tracks tiles are.** Fog/heatmap are precomputed rather than assembled per request (§4.2 — avoiding a `ST_Union()`-per-viewport scaling wall). Filtering live would mean re-fetching and re-parsing every intersecting activity's raw payload *per tile request*, worse at low zoom where one tile can span a huge area.

Fog and heatmap store a **crisp (unblurred) mask per activity, per z14 tile it touches** (`activity_tile_masks` — `services/server/migrations/0005_activity_tile_masks.sql`), rendered once at ingest from the points already in memory (`internal/fog.RenderActivityMasks`, called from `ingest.Process` right after persisting the activity) — no raw-payload re-fetch, and ingest-time rendering cost is proportional to just that one activity's own tiles, independent of how many *other* activities already touch them. One crisp mask feeds both rasters: fog composites active masks with a true per-pixel max and blurs once after; heatmap blurs each mask individually, then sums.

`fog_tiles` is the cached "everyone, whole history, nothing hidden" composite driving the z13→z0 pyramid, and is what an unfiltered request reads. `renderAndStoreTile` composites already-rendered `activity_tile_masks` rows — precise by construction, since `activity_tile_masks` only ever contains tiles the *raw* trajectory actually passed through.

**A filtered request (`from`/`to`, or a new `exclude=id1,id2` for the hidden set) composites on the fly instead** (`internal/fog/filtered.go`'s `RenderFilteredTile`), never touching raw GPS payloads: a cheap existence check against the unfiltered `fog_tiles` aggregate rejects "nothing here" requests for free (filtering only ever removes activities relative to it), one bounded range query fetches every surviving z14 mask under the requested tile in one round trip (not one query per pyramid node — that would scale with `4^(zoom depth)` at low zoom), and the result is downsampled bottom-up in memory from that sparse set, touching only branches that actually have data. A short-lived (45s) in-process cache absorbs the repeated overlapping tile requests one pan/zoom gesture produces. `exclude` has no equivalent on tracks/list/summary — those hide client-side (`tracks.ts`'s `setHiddenTracks`, a MapLibre layer filter); a raster tile has no per-feature identity for that to act on, so exclusion has to reach the server.

**Frontend**: `fog.ts`/`heatmap.ts` take the same `from`/`to` `tracks.ts`'s `trackTileURL` sends, plus `exclude`, and a `refreshFogLayer`/`refreshHeatmapLayer` pair mirroring `refreshTrackLayer`'s `setTiles`-based refetch. `MapView.tsx`'s `maskQuery` reuses `activityQuery` and adds `[...mapHiddenIds]` as `exclude` — the full eye-icon/TYPE/DISTANCE union already computed for `setHiddenTracks`, not just the eye-icon set alone.
...
**Verified against real data, not just synthetic fixtures**: purged the dev database, three synthetic activities across distinct dates and non-overlapping areas confirmed each filter combination (date range alone, exclude alone, both together, unfiltered) precisely — checked by decoding the actual served PNG's alpha channel, not by eye. Then purged again and re-uploaded the real ~2000-file Google Takeout archive (§4.0.2) end to end: all 118 activities rendered masks with zero failures, the map's own Fog-mode requests carried the live `from`/`to`/`exclude` matching the current UI state (confirmed via captured network requests, not assumed from the code alone), and a filtered low-zoom (`z0`) request over the whole dev dataset returned in well under a second including the first, uncached render.
...
The second product pillar (`VISION.md` §4.2/§5.3), computed from `activity_streams`.

**Built so far — ingest-gap fixes and trends.** Scoping the rest of this section against the real ingest code found three gaps blocking most of it: `parse/gpx.go` never read GPX's HR/ cadence/power extension block (§3.4's note above), `moving_seconds` was declared in §3.3 but never populated (no stopped-time detection existed), and no per-point cumulative distance was stored anywhere, which best-effort curves and personal bests both need. `internal/ingest`'s `computeMetrics` now computes `movingS` (a point-to-point speed threshold, ~1.8 km/h, excludes a segment below it from moving time) and a per-point cumulative-distance array (`dist_m`, §3.4) in the same pass that already walked every point for `distanceM` — no second scan. None of this is user-visible on its own; it exists so the curves/bests/load work below isn't also an ingest change when it starts. **Trends** — count/distance/moving-time/elevation-gain per week or month, `GET /v1/activities/trends?bucket=week|month` — is the one piece of this section with its own UI today (`apps/web/src/ui/Trends.tsx`, on `ProfilePage.tsx` below the activity grid): it needed no schema change and reuses §4.8's stat-query shape, so it shipped alongside the ingest fixes rather than waiting on the rest.

**Built next — pace and heart-rate best-effort curves.** `internal/ingest.computeBestEfforts` runs after the `activity_streams` insert, still against `points` in memory (§4.1 step 4/5), walking the same `elapsedS`/`distM` arrays that insert already built. For each of a fixed set of windows (5 s … 1 h, `bestEffortWindowsS`), a two-pointer scan finds the tightest span with duration `>= window` and keeps the best (highest) average over it — average speed for pace, an HR-weighted-by-time average for heart rate. **Not a monotonic deque**: the two-pointer's left bound is monotonically non-decreasing as the right pointer advances (a direct consequence of "the window only needs to get tighter as more slack accumulates further right"), which is already O(n) per window without a deque's extra bookkeeping. Heart-rate curves are skipped entirely for an activity with any point missing `HeartRate` — a partially-covered curve would understate effort rather than report nothing, and sensor coverage is realistically all-or-nothing per activity. Results land in `activity_best_efforts` (`migrations/0010_activity_best_efforts.sql` — a per-activity child table with no `user_id`, the same shape as `activity_tile_masks`), and `GET /v1/activities/best-efforts?metric=pace|heartrate` serves the all-time curve as a plain `MAX` aggregate grouped by window, joined to `activities` for user scoping. `apps/web/src/ui/BestEfforts.tsx`, on `ProfilePage.tsx` below Trends, plots it as an inline SVG polyline (log-scaled x, no charting library). Power curves are not built — same computation, but gated behind an actual audience (power-meter owners) that doesn't justify a third metric yet.

**Built last — personal bests.** `internal/ingest.computeSplits` runs right after `computeBestEfforts`, over the same `elapsedS`/`distM` arrays — the inverse question of a best-effort curve (minimize time to cover a fixed distance, instead of maximize average over a fixed duration), so the two-pointer's roles swap accordingly but the monotonicity argument is identical. Five standard distances (`standardDistancesM`: 1K, 5K, 10K, half marathon, marathon — the same five Strava tracks), no per-activity-type split: `activity_type` has no controlled vocabulary (§4.7), so bucketing by it would fragment the same achievement across e.g. `"run"`/`"running"`/`"trail_running"` rather than unify it. No data-completeness gate either, unlike heart-rate curves — distance and elapsed time have no missing-sensor case. Results land in `activity_splits` (`migrations/0011_activity_splits.sql`, same no-`user_id`-column shape as `activity_best_efforts`), and `GET /v1/activities/personal-bests` serves the all-time record per distance via `DISTINCT ON (distance_m) ... ORDER BY distance_m, seconds ASC` — Postgres's idiom for "the row with the min value per group" in one pass, which also gets *which* activity and *when* for free (the best-efforts `MAX` aggregate above has no such "which activity" to report, so it didn't need this). `apps/web/src/ui/PersonalBests.tsx`, on `ProfilePage.tsx` below BestEfforts, reuses `ActivityGraph.tsx`'s existing stat-card classes rather than a chart — five labeled numbers, not a curve. A distance with no record yet still renders its card with an em dash rather than being omitted, so all five are always visible.

**Not yet built**:

* **Per-activity derived metrics beyond the curves above** — grade-adjusted pace, HR zones and time-in-zone. Not computed or stored anywhere yet. (The per-activity elevation profile itself shipped since this was written — §4.3.2/FR-4.9 — alongside the colored pace/HR bands, not as a separate metric of its own.)
* **Power curves** — same best-effort mechanism as pace/heart rate above, gated on there being cycling/power-meter users to show it to.
* **Training load** — a rolling stress metric over time. Requires a threshold value per sport, either user-entered or estimated from the best-effort curve. Also requires an athlete-settings concept (max/resting HR or FTP) that doesn't exist yet — `users` has no such columns.
* **Recovery inputs (Oura)** — sleep, HRV and readiness are stored against the day, not against an activity, and join to training load in the analysis view. This is the one data source with no geometry at all.

**All of this is derived data** — it must be recomputable from streams, because the definitions will change. If a curve definition changes, recomputation re-parses the raw payload (§4.1 step 6) rather than reading `activity_streams` back out — consistent with why that table has no position columns of its own.
...
**Resolved: no name column.** An early design's row text ("Long ride — Karlštejn") implied an `activities.name` this schema doesn't have (§3.3) — rather than add one, plus the per-format parsing work to populate it (GPX's optional `<name>`, TCX's `<Notes>`, FIT's string fields, and a fallback for the very real case where the source carries none), the list shows `started_at` instead. Already a required column, already indexed (`idx_activities_user_time`), and never missing — no migration, no parser work, no fallback case to design for. The row's primary line is the full start datetime, not a short date alongside a name.
...
- **The daily histogram** (Jan–Nov bars) — a per-day (or per-week) count/distance aggregate across the *whole* visible timeline, independent of whatever sub-range is currently selected. A `GROUP BY date_trunc(...)` query, unrelated in shape to the other two.
  ...
  **The footer header is two corners, not one column.** The left block names the *selection* (its date range, then how many of its days actually have activity — `"N-day range · M active days"`), but no longer repeats km/activity-count/duration/elevation: those already live in `ActivitiesPanel`'s "km loaded" subtext and ACTIVITIES badge (§4.7.1 below is the filtered version of the same count), and the histogram showing them again was the same two numbers twice, not new information. The *pan window* — a different question, since it can show a completely different stretch of history than what's selected — moved out of that column into its own corner rather than staying stacked underneath in a smaller font.
  ...
  **The header is a row above the chart, not beside it; the chart is full width.** The Earlier/Later buttons live in that header row, next to the pan-window label — grouped with the thing they page, rather than flanking the chart as two button columns. `RangePicker.tsx` only takes the `onPan` callback its own drag-the-strip gesture needs; `ActivityHistogram.tsx` renders the buttons. `.activity-histogram` is a column layout, header on top (142px), chart below, so the header's height doesn't squeeze the bars.
...
#### 4.7.4 Edit activity type and description (FR-5.10)

**Built directly on §4.7.2's own resolution, not in tension with it.** §4.7.2 already decided `activity_type` is whatever the source reports, not a controlled vocabulary — a user-typed rename is architecturally identical to what ingest itself already writes there, so nothing about that decision needed revisiting to let a user edit it after the fact. The one genuinely new piece is `description` — a nullable `TEXT` column on `activities`, free text, not a name; `activities` still has no name column — `started_at` is the row's primary line (§4.7).

**`PATCH /v1/activities/{id}`** (`internal/httpapi/activities.go`'s `handleUpdateActivity`) follows `internal/httpapi/account.go`'s `handleUpdateSettings` shape rather than inventing a new one: full-replace-on-save (one Save button commits `activity_type` and `description` together, not per-field), `NULLIF($n, '')` for "empty description means cleared," and ownership-scoped exactly like `track-metrics/{id}` (`WHERE id = $1 AND user_id = $2`, rows-affected checked to return a `404` indistinguishable from "doesn't exist" for a non-owned id). `activity_type` has no such empty-means-unset escape hatch, since the column stays `NOT NULL` — an empty value is rejected with a `400`, not silently kept unchanged. `maxActivityTypeLen` (50, matching the column's own `VARCHAR(50)`) and `maxActivityDescriptionLen` (2000, a generous-but-real free-text bound, the same "reject an obvious mistake, not opine on reasonable length" reasoning `account.go`'s `maxPrivacyTrimM` already uses) are both enforced server-side and mirrored client-side in the dialog, so a caller sees the limit before submitting, not only after a rejected request.
...
**Frontend: the row's icon order is Visible, Edit, Delete — a direct product decision, not alphabetical or feature-age order** (`ActivitiesPanel.tsx`; the eye icon moved before the pencil icon to make room). Confirmed directly with the user, not assumed: a delete has no undo, so clicking the trash icon opens a new `ConfirmDialog.tsx` — deliberately generic, not delete-specific, reusing the same `<dialog>`/`showModal()` shape `PlaceholderNotice.tsx`/`EditActivityDialog.tsx` already established, so a future destructive action can reuse it rather than growing its own confirm UI. On confirm, `MapView.tsx`'s `handleActivitiesDeleted` reuses `handleUploaded` verbatim (deleting changes distance/duration totals and the histogram, unlike editing, which needs only a plain `reload()`), and additionally clears `focusedActivityId` if it pointed at any now-deleted activity — left dangling, the colored-zone-segments/pace-profile effect would keep trying to fetch `track-metrics` for an activity that no longer exists. `checkedActivityIds`/ `hiddenActivityIds` are cleared too for tidiness, though a dangling id in either is already harmless (both are filtered against the freshly-reloaded activities list wherever they're read). Takes an array of ids, not one — §4.7.6's "Delete group" batches a whole checked group through the same function rather than calling a single-id version once per activity, so one combined refresh happens regardless of how many activities were deleted at once. `handleActivityDeleted(id)` (still used by the per-row delete icon) is now a thin wrapper calling this with a one-element array.

#### 4.7.6 Header toolbar — select-all, Type dropdown, group actions (FR-5.2/FR-5.7/FR-5.12/FR-5.13)

**Direct product decision, not additive: this toolbar replaced the old per-row-only selection model, not sat alongside it.** The panel head's "Filter" toggle button and the footer's "Select all"/"Clear" text button are both gone. The footer keeps its summary text and "Show selected" button, which the new toolbar has no equivalent for.

**Distance is its own standalone, always-visible section, not folded into the Type dropdown** — a control reachable without a click shouldn't be buried behind one. **The toolbar sits directly above the row list**, not up by the heading, and is *column-aligned* with the rows beneath it — not just visually nearby, but literally sharing the same flex geometry so each toolbar control sits directly above its row-level counterpart.

**The toolbar mirrors `.activities-panel__row`'s own flex layout column for column** — same `gap: 10px; padding: 10px 16px`, same child widths in the same order, so it reads as that list's header rather than a floating control strip:

| Row column | Toolbar column | Width |
| :-- | :-- | :-- |
| Checkbox | Select-all checkbox | 15px — literally the same `.activities-panel__checkbox` class, not a lookalike |
| Row icon | Empty spacer | 28px, matching `.activities-panel__icon` |
| Title/meta text | Empty spacer | `flex: 1; min-width: 0`, matching `.activities-panel__text` — this is what absorbs narrowness at the panel's 260px minimum, the same way a row's own title text does |
| TYPE label | Type dropdown trigger | `flex: none; width: 5.5rem`, matching `.activities-panel__type`'s own `max-width` — the *cap*, not each row's actual (variable, content-dependent) width, since that's the only stable target multiple rows of differing type-label lengths can all align against |
| Visible (eye icon) | Group visible | 24px — literally the same `.activities-panel__visibility` class |
| Edit (pencil icon) | Empty spacer | 24px, matching `.activities-panel__edit` — no bulk-edit action exists, so this column stays empty rather than collapsing, which is what keeps Delete group aligned under Delete |
| Delete (trash icon) | Delete group | 24px — literally the same `.activities-panel__delete` class |

Verified live via `getBoundingClientRect()` diffs, not just eyeballed: the checkbox, Visible, and Delete columns land at *exactly* 0px offset from their row counterparts; the Type dropdown's right edge lands exactly on the row TYPE column's own right edge (its left edge varies, since a row's actual TYPE text — "Run", "Bike" — is often narrower than the 5.5rem cap and left-aligns within that reserved space, which is expected: the toolbar aligns to the column's stable *right* boundary, shared with the fixed-width icon buttons that follow it, not to each row's own variable-width label).

**Group visible and Delete group reuse the exact per-row icon button classes (`.activities-panel__visibility`/`.activities-panel__delete`), icon-only with no text label** — the same as in the list. Group visible's own icon reflects state the same way the per-row eye icon does: open when the checked group has nothing hidden, closed (slashed) when any of it is — computed in `ActivitiesPanel.tsx` as `groupHasHidden = checkedActivities.some(a => hiddenIds.has(a.id))`. The Type trigger keeps its "Type ▾" text label — only the two bulk-action buttons were asked to go icon-only, not the filter trigger.

**The Type dropdown now holds only TYPE checkboxes; Distance is a sibling, not a child.** `DistanceFilter.tsx` (renamed from `ActivityFilters.tsx`, which now does the one job its name should have always implied) renders standalone between the subtext line and the toolbar, guarded by the same `bounds && bounds.min < bounds.max` check the old combined component already had — returns `null` rather than rendering anything when there's nothing meaningful to show a slider over. TYPE's own checkbox-list markup (`.activity-filters__type-list`/ `__type-item`, `checked` meaning "shown", scrolling via `max-height: 12rem; overflow-y: auto` instead of the old chip grid's "Show all (N)") now lives inline in `ActivitiesPanel.tsx` itself — small enough, once it's just checkboxes with no slider alongside it, not to need its own component file. `.activities-panel__type-panel` anchors `right: 0` under its trigger, not `left: 0` — the trigger now sits mid-row (above the TYPE column, not at the panel's left edge), so right-anchoring is what keeps the dropdown from overflowing the panel's own narrow end.

**The header's master checkbox is a real tri-state control, not two swapped button labels.** `allChecked`/`someChecked` are computed from `checkedActivities.length` against the currently *listed* (already TYPE/DISTANCE-filtered) `activities.length` — `indeterminate` has no JSX prop (it's a DOM property, not an HTML attribute), so a ref + a `useEffect` sets it imperatively whenever `someChecked` changes. Clicking it when unchecked-or-indeterminate calls the existing `onSelectAll`; clicking it when fully checked calls the existing `onClear` — both callbacks are exactly what the old footer buttons already called, unchanged in `MapView.tsx`; only the trigger moved.

**Group visible (FR-5.12) — confirmed directly as a toggle, not an "isolate" pattern.** The alternative considered was showing only the checked group and hiding everything else, a "solo" pattern from layer-based tools — rejected because it also changes *unchecked* rows' state, a bigger behavior change than a bulk version of the existing per-row eye icon needed to be. `MapView.tsx`'s `toggleGroupVisibility`: if any checked id is currently in `hiddenActivityIds`, remove every checked id from it (show the whole group); otherwise add every checked id (hide the whole group). One `useCallback`, no new component — the existing eye icon's own `hiddenActivityIds` Set is the only state this touches, so `tracks.ts`'s layer-filter application and Fog/Heatmap's hidden-activity exclusion (§4.2.3) both already handle the result correctly with no changes of their own.

**Delete group (FR-5.13) reuses §4.7.5's `DELETE /v1/activities/{id}` per activity, not a new bulk endpoint.** `ActivitiesPanel.tsx`'s confirm handler awaits each `deleteActivity(id)` call **sequentially**, not `Promise.all` — a checked group is a handful of rows a user selected by hand, not a bulk-import-scale operation, so there's no latency reason to parallelize, and sequential avoids N concurrent deletes each independently marking/enqueueing against the same user's `fog_tiles` rows at once. Once every delete lands, one call to `onActivitiesDeleted(ids)` (§4.7.5's now-batched `handleActivitiesDeleted`) does the combined refresh — never one refresh per deleted activity. The confirm message pluralizes correctly for a one-activity group ("its track…") versus a larger one ("their tracks…").

**Verified live**: confirmed the corrected DOM order (head → subtext → Distance → toolbar → list → footer) and that Distance renders with no click needed. Confirmed Group visible/Delete group render with no text, matching the row icons exactly. Confirmed pixel alignment via `getBoundingClientRect()` as described above. Confirmed the Type dropdown lists every distinct type as a checkbox, unchecking one narrows the row list and re-checking restores it, and the dropdown closes on an outside click with no Distance control leaking into it. Confirmed the header checkbox checks/unchecks all rows and reports `indeterminate` correctly for a partial selection. Confirmed Group visible, scoped to a genuinely partial selection (not accidentally the select-all checkbox — both share `.activities-panel__checkbox`, so the live check had to scope its selector to `.activities-panel__list` to test this correctly), hides exactly the checked activity and shows it again on a second click. Confirmed Delete group's confirm dialog names the correct count/distance and singular/plural wording, Cancel leaves the group untouched, and confirming removes exactly the checked ones. Confirmed no horizontal overflow at a 390px mobile viewport with the same column alignment holding.
...
**"Claim vs. create" is the key design decision.** The very first signup ever (`SELECT EXISTS (SELECT 1 FROM users WHERE password_hash IS NOT NULL)`) updates the seeded placeholder user's row in place — same `id`, new `email`/`password_hash` — instead of inserting a new one (`claimOrCreateUser`). Every activity uploaded before accounts existed stays attached to the same account with zero migration or backfill step; subsequent signups behave normally (a plain new row, unique-email-enforced). Verified directly: the placeholder `id` was unchanged after claiming, and an authenticated `GET /v1/activities` immediately returned activity history uploaded before the account existed.
...
**Deliberately simple, matching the instruction that built it**: no OAuth, no email verification. Password reset is built now — §4.11. Rate limiting is no longer a blanket gap either — §4.10's demo endpoint has one, since it is the one place that couldn't be left alone, and §4.11's forgot-password endpoint needed the exact same treatment for the exact same reason.

**Test suites need to authenticate too.** `smoke.mjs`/`build.mjs` each spawn their own dev/ preview server and a fresh Playwright browser context with no session cookie — with the app now gated behind `AuthGate`, both suites sign in a dedicated `smoke-test@fitmap.local` account via `context.request` (signup, or login if a prior run already claimed that email) before navigating. A Docker-only gotcha worth documenting: reaching `api` over service-name DNS (`http://api:8080`) is *cross-site* from the test's own dev server (`http://localhost:5180`) as far as `SameSite=Lax` cookies are concerned — different hostnames, even both being local — so the browser silently drops the session cookie on every request after sign-in, and the suite hangs waiting for a map that never authenticates. `test` uses `network_mode: "service:api"` instead, so both are reachable as `localhost` at different ports only — the same relationship real dev already has between the `web` and `api` services — plus adding the two suites' ports to `corsAllowedOrigins`.

### 4.10 The no-signup demo

**Built.** `VISION.md` §8.2: "a no-signup, drag-a-file-in, see-your-fog-map page" — the single asset that section calls "the single best asset in this plan." Mechanism decided explicitly rather than assumed: reuse §4.9's entire auth/session/upload/ingest/fog pipeline unchanged, behind a real but ephemeral account, rather than a second, from-scratch client-side parse-and-render path. The latter would have meant reimplementing GPX/FIT/TCX parsing and fog rasterization a second time in a different language, with two implementations to keep in sync forever after — a much larger and more fragile undertaking for the same visible result.

**`POST /v1/auth/demo` (`handleDemoStart`, auth.go) creates a real `users` row** — same table, same columns, same session mechanism as a real account — marked ephemeral via a nullable `demo_expires_at` column. Its `email` is a synthetic, never-shown placeholder (`demo-<uuid>@fitmap.invalid` — `.invalid` is the RFC 2606 TLD reserved for addresses guaranteed never to be real); `handleMe` strips it to `""` in every response rather than ever letting it reach the frontend. Because it's a real account, every existing route — upload, ingest, tiles, the activity graph — runs completely unchanged for a demo visitor; nothing in `internal/httpapi`'s other handlers has a demo-specific branch at all. `demoSessionTTL` (24h) is both the account's expiry and its session cookie's own TTL — long enough to try the app and share a link same day, short enough to bound storage from anonymous traffic that never converts.

**Cleanup: internal/worker/demo_purge.go, on a second ticker in `worker.Run`** (5 minutes, far coarser than the job-queue poll — TTLs are hours, not milliseconds). Deletes DB rows in batches of 100 via `DELETE FROM users WHERE demo_expires_at < NOW()`, which cascades for free (every `user_id` foreign key in the schema is already `ON DELETE CASCADE`). Object storage has no foreign keys, so raw uploads and rendered fog/heatmap tile pyramids — `raw/{userID}/`, `fog/{userID}/`, `heatmap/{userID}/`, all namespaced by user id — are swept first via a new `Store.RemoveByPrefix`. **Known, accepted gap**: `activity-masks/ {activityID}/` (§4.2.3's per-activity masks) is namespaced by *activity* id, not user id, so these small per-tile crisp masks are left orphaned rather than tracked and swept per activity — not a correctness bug (nothing looks them up once `activity_tile_masks`' own rows are gone with the cascade), just a lower-value cleanup deferred until it measures as worth doing.

**Rate-limited per IP (`demoLimiter`, auth.go)** — 5 demo accounts per hour, a plain in-memory fixed-window counter with lazy eviction of stale entries, not a library, since this is the one endpoint of the whole "no rate limiting" gap that couldn't be left alone: it's reachable with no session or credentials at all, and creates real rows and accepts real uploads. Keyed by `r.RemoteAddr`'s host, not `X-Forwarded-For` — no reverse proxy sits in front of this server today, so that header would just be an attacker-controlled value.

**Converting a demo into a real account preserves it, rather than starting over.** Signing up while a live demo session is held (`handleSignup` checks `currentDemoUserID` first) claims that same row in place via `claimDemoUser` — sets `email`/`password_hash`, clears `demo_expires_at` — instead of `claimOrCreateUser`'s placeholder-or-new-row logic, so whatever was uploaded during the demo survives becoming a real account under the same id.

**`UserMenu.tsx`'s "Demo session — save this" reuses `AuthGate` itself, full-page, rather than a second form.** The demo's uploaded data is rows in Postgres tied to the still-valid session cookie, never at risk from what the frontend happens to have on screen, so there's no need to preserve any client-side state across the swap except `MapView`'s own transient view state (camera, filters, selection), which resets on cancel. "Save this" calls `requestUpgrade()` on `AuthContext`, which `App.tsx` turns into a third top-level render branch — the same `AuthGate`, in signup mode, with an `onCancel` prop that swaps `AuthGate`'s "Try it now — no signup" button for a "← Back" link (starting a second demo mid-demo would just abandon the first) and puts the whole thing back once either `onCancel` fires or signup succeeds.

**`SessionUser` is a real discriminated union, `AuthUser | DemoUser` (`api.ts`), not `AuthUser` plus an `isDemo: boolean` flag.** `DemoUser` is deliberately empty — a demo session carries nothing else worth exposing — which is what lets `'email' in user` narrow a `SessionUser` correctly with no explicit tag needed. The backend's wire format is unaffected (`{email, isDemo}` JSON); the boolean survives only inside `api.ts`'s own parsing, deciding which of the two shapes to construct — `login`/`signup` return a plain `AuthUser` (a successful one is never a demo account) and `startDemo` a plain `DemoUser`, so no caller has to re-narrow a flag that couldn't actually vary for them.
...
**Verified live**, not just unit-level: `POST /v1/auth/demo` → real session → authenticated upload → real `raw/{userID}/...` object in MinIO and a real `activities` row; the 6th demo start from one address in an hour returns `429`; signing up on a live demo session claims the same `id` (checked directly against the row, not inferred from the response); the purge sweep, exercised with a shortened interval, deleted both the DB row and the MinIO object for an expired demo account, confirmed via a direct `StatObject` call returning "key does not exist" rather than trusting an HTTP 403 (which a private bucket returns for anonymous requests whether or not the object exists); and the full click-through flow (`Try it now` → map renders → account menu shows "save this" → submitting the form → reload stays signed in as the new real account) end to end in a live browser.
...
**Verified live, end to end**: signed up, opened Settings from the account menu, uploaded an avatar (confirmed it appears immediately in the account-menu button, everywhere, with no reload), set Name and Country (`US`) and a new Privacy Trim value, saved — confirmed the Activities panel's row distances, its footer's "loaded"/selected-group summaries, the distance-filter slider, and the Profile page's stat cards, Trends tooltip, and Best Efforts all switched to miles/feet immediately; confirmed Personal Bests' race-distance labels did not change. Also confirmed no console errors across the whole flow.
