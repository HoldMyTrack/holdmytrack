...
**Keeping the window current without a live per-request check**: an activity entering the window (a fresh upload) is handled by the exact same ingest-time dirty-marking Fog already has — nothing new there. An activity *leaving* the window is a pure function of time passing, which ingest/delete events have no reason to ever notice, so `internal/worker`'s `refreshHeatmapWindows` sweeps every user once a week (`heatmapRefreshInterval`), marking every one of their tiles dirty (`ingest.MarkAllFogTilesDirty`) and re-enqueuing the same `render_fog` job ingest itself uses. The trade-off is explicit: an aged-out activity takes up to a week to cool off, not exactly 365 days to the hour — accepted, since nothing about this feature is watched closely enough in real time for the difference to matter. Fog's own aggregate has no aging concept, so recomputing it on the same weekly sweep is pure, harmless duplicate work, not a second code path to maintain.
