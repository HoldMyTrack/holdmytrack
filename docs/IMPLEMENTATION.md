...
**`POST /v1/auth/demo` (`handleDemoStart`, auth.go) creates a real `users` row** — same table, same columns, same session mechanism as a real account — marked ephemeral via a nullable `demo_expires_at` column. Its `email` is a synthetic, never-shown placeholder (`demo-<uuid>@fitmap.invalid` — `.invalid` is the RFC 2606 TLD reserved for addresses guaranteed never to be real); `handleMe` strips it to `""` in every response rather than ever letting it reach the frontend. `demoSessionTTL` (24h) is both the account's expiry and its session cookie's own TTL — long enough to try the app and share a link same day, short enough to bound storage from anonymous traffic that never converts.

**Revised, docs/ROADMAP.md's "Email verification + demo without real ingest": a demo account is now read-only, not "every route runs unchanged."** The original design reused every route completely unchanged for a demo visitor, upload and sync included — an anonymous, no-credential endpoint that could trigger real per-activity server compute (parsing, fog/heatmap mask rendering) for data guaranteed to be purged a day later. `requireNotDemo` (auth.go) now rejects upload, sync, edit, delete, avatar, and settings for a demo session regardless of what a client attempts; `requireVerified` still passes a demo session through for everything else (list, tiles, the activity graph), since the verification gate itself was never meant to apply to one. In place of real ingest, `handleDemoStart` calls `demo_presets.go`'s `seedDemoPresets` — two fixed, hand-written GPX payloads ingested through the real `internal/ingest.Process` pipeline (so fog/heatmap masks render for them exactly like a real upload would), not a hand-rolled DB insert that would need to reimplement that rendering step. The full curated, geo-selected preset library `docs/ROADMAP.md`'s own item describes is explicitly still out of scope — this is the smallest thing that makes "view-only preset activities" real. `ActivitiesPanel.tsx`'s `readOnly` prop and `UploadPanel.tsx`'s own (both driven by `MapView.tsx`'s `isDemo`) disable, rather than hide, the corresponding controls — with a `title` explaining why — so a demo visitor sees that editing/deleting/uploading exist rather than wondering, which is closer to the point of a demo than hiding the feature would be. Either way this is a UX courtesy, not the actual enforcement, which is `requireNotDemo` alone.
...
**Converting a demo into a real account no longer preserves it in place — there is nothing left worth preserving.** The original design claimed the live demo session's row in place (`claimDemoUser`, checked via `currentDemoUserID`), so whatever was uploaded during the demo survived becoming a real account under the same id. **Revised**: since a demo account can no longer hold anything but the fixed preset activities `seedDemoPresets` seeded it with — never real, uploaded data — there is nothing to carry forward. `handleSignup` treats a demo-session signup exactly like a cold one: a plain new row, gated by email verification like any other. The preset activities are simply left behind on the demo account, which `internal/worker/demo_purge.go` purges on its own schedule regardless of whether the person went on to create a real account.
