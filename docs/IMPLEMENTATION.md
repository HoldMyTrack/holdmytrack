...
#### 4.0.1 Import UI: batching, non-blocking status, a persistent history, Files/Sync tabs, and "View on map"

**Built.** The single-file `UploadWidget.tsx` (drag one, or click to pick one, watch one hover popover) is gone, replaced entirely by `ImportPanel.tsx` (renamed from `UploadPanel.tsx` — `ROADMAP.md`'s "Rename Upload to Import" item, closed by this pass) — a dropdown panel opened from the header's "Import" button, with two tabs: **Files** (this section's original scope, unchanged below) and **Sync** (§4.0.1.1).
...
`VISION.md` §4.2's "Export" pillar, first slice — see §5.5 for why this is client-side rather than a server-side render. The user frames a region of the live map manually rather than exporting an automatic fit of whatever happens to be on screen: `ExportButton.tsx` opens `ExportPresetDialog.tsx`, a grid of platform image-size presets from `apps/web/src/map/exportPresets.ts` — `EXPORT_PLATFORMS` (Instagram/Facebook/X) as columns, `EXPORT_PRESET_ROWS` (Square/Portrait/Landscape/Story) as rows, dimensions curated from Hootsuite's social-media-image-sizes guide with profile-picture sizes excluded — plus a Custom button below the grid, with no fixed dimensions. A cell with no matching preset for its (platform, row) pair (e.g. X × Story) renders empty rather than a disabled placeholder. Picking a cell or Custom shows `ExportFrame.tsx`, an overlay over the live map with a frame matching the chosen shape, marked only by a dashed border (`.export-frame__box`) — no darkening: the live map is fully visible and interactive everywhere, including right up to the frame's own edge. The outer `.export-frame` wrapper is `pointer-events: none`, so clicks/drags reach the map through it everywhere except the frame box itself, its corner handle (Custom only), and its Close/Capture buttons, which are the only children that opt back into `pointer-events: auto`. The frame box's *entire interior* is the move-drag target — dragging anywhere inside it repositions the frame anywhere over the map; dragging the live map means grabbing anywhere outside the box, the same gesture as ordinary map panning, since there's no separate edge-only hit zone. Frame position/size is stored as fractions of the map container (`FrameRect` in `ExportFrame.tsx`), not raw pixels, via CSS custom properties (`--frame-x/y/w/h`), so it stays correctly proportioned across a window resize with no recompute wiring; drag math (pointer-capture, `RangePicker.tsx`'s established pattern) reads pixels back from `getBoundingClientRect()` only at drag time and at capture. Close (cancels, returns to the normal map view) sits immediately left of Capture, both in the frame's own top-right corner.

Clicking the frame's capture button calls `exportFramedImage` (`apps/web/src/map/exportMap.ts`), which:

1. Builds a second, temporary, hidden `MapLibreMap` via the shared `renderOffscreen` helper, in an off-screen container (`position: fixed; left: -99999px` — not `display: none`, which leaves an element with no real layout box for WebGL to size a canvas against), sized to `EXPORT_SUPERSAMPLE`× (3×) the *live map container's own on-screen CSS-pixel size* — not a fixed target width — so the offscreen canvas stays pixel-registered to the live view for the crop math below. Camera (`center`/`zoom`/`bearing`/`pitch`) is copied from the live map exactly, for true WYSIWYG-relative-to-the-frame output; the instance is constructed with `canvasContextAttributes: { preserveDrawingBuffer: true }` (nested there, not a top-level `MapOptions` field, in the installed maplibre-gl 6.9.0 — confirmed against its own type declarations), which the live map deliberately never sets itself, confining that real, if usually small, perf cost to the temporary instance.
...
3. Crops the offscreen canvas to the frame's proportional rect: `(frameRect - containerRect) × EXPORT_SUPERSAMPLE × backingRatio`, where `backingRatio = canvas.width / tempWidth` is read back from the rendered canvas itself rather than trusted from `window.devicePixelRatio` at call time (sidesteps drift between the two, e.g. browser zoom changing DPR mid-session) — a `drawImage` onto an intermediate canvas.
4. Scales that crop to the output: a preset's exact declared pixel dimensions (`drawImage` onto a canvas sized to them), or for Custom, proportionally so the crop's longer side hits `EXPORT_LONG_SIDE_PX` (2400, the same "generously larger than a screenshot" ceiling this module always used, renamed from the old fixed-width constant since a portrait Custom frame's longer side is its height just as validly).
5. Reads `finalCanvas.toBlob('image/png')`, tears the offscreen instance and container down. `ExportFrame.tsx`'s capture button triggers the browser download via a blob URL and a synthetic `<a download>` click — no server round trip, so no cost to rate-limit — named `fitmap-{date}.png` regardless of preset.

Attribution/logo bake-in (`docs/KNOWN_ISSUES.md`'s attribution gap, `docs/ROADMAP.md`'s FitMap-logo item, both still open) belongs on the *final* canvas in step 4 above, not the offscreen `renderOffscreen` canvas from step 1 — frame position and preset scaling both happen after that step, so a mark drawn any earlier would land in the wrong place or the wrong size in the actual output.

**Not in this slice**: colored zone segments (§4.3.1 — a single-focused-activity view, narrower than "export my map"), vector/SVG output (raster only), story cards/animated reveals (`VISION.md` §4.2's other two "Export" items, unbuilt), and full mobile-touch polish of the frame's own handles/dialog layout (the drag/resize interaction is pointer-events-based — mouse and touch unified — from the start, but comfortable touch-target sizing and small-screen dialog layout are deferred to the mobile-browser-support pass `docs/ROADMAP.md` Phase 3 already tracks).
...
**`requireNotDemo` (auth.go) rejects upload, sync, edit, delete, avatar, and settings for a demo session regardless of what a client attempts**; `requireVerified` still passes a demo session through for everything else (list, tiles, the activity graph), since the verification gate was never meant to apply to one. `ActivitiesPanel.tsx`'s `readOnly` prop and `ImportPanel.tsx`'s own (both driven by `MapView.tsx`'s `isDemo`) disable, rather than hide, the corresponding controls — with a `title` explaining why — so a demo visitor sees that editing/deleting/uploading exist rather than wondering. Either way this is a UX courtesy, not the actual enforcement, which is `requireNotDemo` alone.
...
### 4.12 Account settings (Avatar, Name, Country, Timezone, Privacy Trim)

`users` carries `display_name`, `country` (`CHAR(2)`, ISO 3166-1 alpha-2), `avatar_key`/`avatar_content_type`/`avatar_updated_at` (`migrations/0001_init.sql`), `timezone` (`TEXT NOT NULL DEFAULT 'UTC'`, `migrations/0021_user_timezone.sql`), alongside `privacy_trim_m` (`INT NOT NULL DEFAULT 200`) — this is the first UI that lets anyone change any of them.

**Timezone closes the UTC-day-bucketing gap** documented at length in §4.7's histogram note below — see that section for the actual query-side fix. This section covers how the value itself gets set and edited. Auto-resolved client-side at signup (`Intl.DateTimeFormat().resolvedOptions().timeZone`, `api.ts`'s `signup()`) and sent as an optional `timezone` field on `POST /v1/auth/signup`; `auth.go`'s `handleSignup` validates it via `time.LoadLocation` and falls back to `'UTC'` on anything missing or invalid, rather than rejecting the signup over a browser quirk — a sensible default, not an authoritative fact about the account, the same treatment `docs/BRAINSTORM.md`'s Country-at-signup idea proposes for that field. `time/tzdata` is blank-imported in `cmd/fitmap/main.go` so `time.LoadLocation` works regardless of whether the deployed distroless image happens to carry `/usr/share/zoneinfo`. Unlike Country, there is no "unset" state — the column is `NOT NULL DEFAULT 'UTC'`, and `handleUpdateSettings` rejects an empty/unloadable value with a 400 rather than treating it as "clear," since the Settings field always submits its own current selection. `SettingsPage.tsx`'s Timezone field is a `<select>` populated from `Intl.supportedValuesOf('timeZone')` (`ui/timezones.ts`) — read live from the browser's own tz database rather than a generated static list, unlike `countries.ts`'s ISO codes, since an IANA zone name needs no separate display-name lookup step the way a country code does.
...
**Signed-out browsing is unaffected** — there is no `MapView` mount without an active session at all today (`App.tsx`'s signed-out branch renders only `AuthGate`), so this resolution chain never runs for a signed-out visitor. This is now the decided, permanent design on web: anonymous map browsing was considered and rejected in favor of pushing new visitors toward the one-tap Demo account (`VISION.md` §8.2), which shows the actual product rather than a bare basemap. Android's own shell still renders the bare basemap before login today (`apps/android/docs/IMPLEMENTATION.md` §1.2) — `apps/android/docs/ROADMAP.md`'s "Gate the map behind login" item tracks bringing it in line with web's behavior.
