...
**Fog (`GET /tiles/v1/fog/{z}/{x}/{y}.png`) always serves the precomputed aggregate** — `fog_tiles.object_key`, the "everyone, whole history" composite `renderAndStoreTile` builds and keeps up to date from `activity_tile_masks` at ingest/delete time. There is no per-request filtering path for Fog at all; a request always gets exactly what's stored.

**Heatmap (`GET /tiles/v1/heatmap/{z}/{x}/{y}.png`) always takes the on-the-fly filtered path instead** (`internal/fog/filtered.go`'s `RenderFilteredTile`), with one fixed filter the handler computes itself — `{From: time.Now().AddDate(0, 0, -fog.HeatmapWindowDays)}` — never a client-supplied date range or exclude list; query parameters on this endpoint are ignored entirely. A relative rolling window like this is inherently self-bounding (activities in the last 365 days cost roughly the same query size whether the account is one year old or ten), which is what makes always taking the filtered path affordable here, unlike an arbitrary user-chosen range would be. `RenderFilteredTile` composites straight from `activity_tile_masks` — a cheap existence check against Fog's own unfiltered aggregate rejects "nothing here" requests for free, one bounded range query fetches every surviving z14 mask under the tile in one round trip, and the result is downsampled bottom-up from that sparse set. A short-lived (45s) in-process cache absorbs the repeated overlapping tile requests one pan/zoom gesture produces.
