...
**Revised again: `POST /v1/auth/demo` (`handleDemoStart`, auth.go) no longer creates a `users` row at all.** The paragraph below describes the original mechanism (a fresh, ephemeral row per visitor); it now just opens a session against one persistent, shared `DemoCustomerUserID` constant (`demo_presets.go`, matching the fixed id `migrations/0017_demo_customer_user.sql` inserts). Its `demo_expires_at` is a fixed far-future timestamp rather than `NULL` — `isDemo := demoExpiresAt != nil` (auth.go) needs it non-`NULL` to keep the account read-only (`requireNotDemo`) and verification-exempt (`requireVerified`), while a value that far out never matches the purge sweep's `< NOW()` condition below, so the account itself is never deleted. `demoSessionTTL` (24h) now bounds only each visitor's own session cookie — a fresh `POST /v1/auth/demo` call just opens another session against the same account, and any number of visitors can hold one at once, all seeing identical data. Its `email` (`demo-customer@fitmap.invalid`) is the same kind of synthetic, `.invalid`-TLD placeholder the original per-visitor design used, just one fixed value instead of a fresh one per row; `handleMe` still strips it to `""` before it ever reaches the frontend.

**Original mechanism (superseded by the paragraph above, kept for context): `POST /v1/auth/demo` created a real `users` row** — same table, same columns, same session mechanism as a real account — marked ephemeral via a nullable `demo_expires_at` column, with `demoSessionTTL` (24h) as both the account's expiry and its session cookie's own TTL.

**Revised, docs/ROADMAP.md's "Email verification + demo without real ingest": a demo account is now read-only, not "every route runs unchanged."** The original design reused every route completely unchanged for a demo visitor, upload and sync included — an anonymous, no-credential endpoint that could trigger real per-activity server compute (parsing, fog/heatmap mask rendering) for data guaranteed to be purged a day later. `requireNotDemo` (auth.go) now rejects upload, sync, edit, delete, avatar, and settings for a demo session regardless of what a client attempts; `requireVerified` still passes a demo session through for everything else (list, tiles, the activity graph), since the verification gate itself was never meant to apply to one. **Revised again: seeding moved from per-request to a one-time CLI step, and grew from 2 presets to a real 611-activity history.** `handleDemoStart` no longer calls any seeding function at all. Instead, `demo_presets.go`'s `SeedDemoCustomer` — invoked by the `seed-demo-customer` subcommand in `cmd/fitmap/main.go`, run once out of band (e.g. at deploy time), never per visitor — walks 611 embedded GPX files (`//go:embed demo_data/*.gpx`, about 7 months of a single persona's dog walks, bike rides, local errands, and a few multi-day trips) and ingests each one through the same real `internal/ingest.Process` pipeline a real upload uses, exactly as the original 2-preset design did. `ON CONFLICT (user_id, source, external_id) DO NOTHING` (§4.1's idempotency invariant) makes re-running the seed command safe — already-ingested files are skipped, not duplicated — which is what makes a one-time-at-deploy seed workable at all: re-running it after a redeploy just confirms everything is still there. Re-seeding per visitor, the way the original 2-file design worked, would not scale to this file count — `handleDemoStart` returning only after 611 sequential parse+DB+fog-render steps is exactly the cost this redesign avoids. The original per-preset "curated, geo-selected library" idea (`docs/ROADMAP.md`'s own item) does not carry over cleanly to one shared account — a shared account can't show different data per visitor's location — and is now noted there as superseded rather than deferred. `ActivitiesPanel.tsx`'s `readOnly` prop and `UploadPanel.tsx`'s own (both driven by `MapView.tsx`'s `isDemo`) disable, rather than hide, the corresponding controls — with a `title` explaining why — so a demo visitor sees that editing/deleting/uploading exist rather than wondering, which is closer to the point of a demo than hiding the feature would be. Either way this is a UX courtesy, not the actual enforcement, which is `requireNotDemo` alone.
...
**Revised again: this sweep now has nothing to act on under the current design.** It still runs, unmodified, and would still matter if a future change reintroduced per-visitor demo rows — but the shared `DemoCustomerUserID` account's far-future `demo_expires_at` (see above) means its own `WHERE demo_expires_at < NOW()` never matches it, by design, not by coincidence.
...
**Converting a demo into a real account no longer preserves it in place — there is nothing left worth preserving.** The original design claimed the live demo session's row in place (`claimDemoUser`, checked via `currentDemoUserID`), so whatever was uploaded during the demo survived becoming a real account under the same id. **Revised**: since a demo account can no longer hold anything but the preset activities it was seeded with — never real, uploaded data — there is nothing to carry forward. `handleSignup` treats a demo-session signup exactly like a cold one: a plain new row, gated by email verification like any other. **Revised again**: the reasoning holds even more directly now — the shared `DemoCustomerUserID` account belongs to no one visitor, so "leaving its data behind" isn't purging a personal copy the way it was under the per-visitor design, it's simply that a session against a shared account was never personal data to begin with. `internal/worker/demo_purge.go` no longer has anything to purge here either way (see above).

**`UserMenu.tsx`'s "Demo session — save this" still reuses `AuthGate` itself, full-page, rather than a second form** — that mechanism is unaffected by the revision above, only what signing up *does* changed. "Save this" calls `requestUpgrade()` on `AuthContext`, which `App.tsx` turns into a third top-level render branch — the same `AuthGate`, in signup mode, with an `onCancel` prop that swaps `AuthGate`'s "Try it now — no signup" button for a "← Back" link (starting a second demo mid-demo would just abandon the first) and puts the whole thing back once either `onCancel` fires or signup succeeds.
...
**Verified live at the time**, describing the original design: `POST /v1/auth/demo` → real session → authenticated upload → real `raw/{userID}/...` object in MinIO and a real `activities` row; the 6th demo start from one address in an hour returns `429`; the purge sweep, exercised with a shortened interval, deleted both the DB row and the MinIO object for an expired demo account, confirmed via a direct `StatObject` call returning "key does not exist" rather than trusting an HTTP 403 (which a private bucket returns for anonymous requests whether or not the object exists). The rate-limit and purge-sweep findings still hold unchanged. **No longer true**, per the revision notes above: a demo account no longer accepts an authenticated upload at all (`requireNotDemo` rejects it), and signing up on a live demo session no longer claims the same `id` — it creates a fresh, separately-verified account instead. Re-verified live for that design (still describing the 2-preset era): `POST /v1/auth/demo` seeds two real preset activities via `ingest.Process` (confirmed via `GET /v1/activities` returning both, with real computed distance/duration); `POST /v1/activities/upload`/`sync/activities`/`PATCH`/`DELETE` on a demo session all return `403 {"error":"demo_read_only"}`; and the click-through flow (`Try it now` → map renders with the two presets → account menu shows "save this" → submitting the form → reload stays signed in as a new, separately unverified real account, not the demo's own id) end to end against a live stack.

**Re-verified live again, for the persistent shared-account design**: ran `fitmap seed-demo-customer` against a fresh stack — all 611 activities ingested (`ingested: 611, already_present: 0, failed: 0`); ran it a second time and confirmed a clean idempotent no-op (`ingested: 0, already_present: 611`). Called `POST /v1/auth/demo` from two separate cookie jars ("two visitors") — both received distinct session tokens for the same `DemoCustomerUserID`, and `GET /v1/activities` returned the identical 611 rows through either session. Confirmed the write gate still holds on this account (`403 demo_read_only` on upload). Confirmed the purge worker's own query (`demo_expires_at IS NOT NULL AND demo_expires_at < NOW()`) matches zero rows for this account, since its `demo_expires_at` is fixed in the year 9999.
