...
- [ ] A signed-in account with zero activity history yet (nothing to fly to) falls back to their account's **Country** setting (FR-1.7 — already collected, currently used only for units) at a country-level zoom, instead of the Ohio point — a far more relevant default than an arbitrary fixed coordinate.
