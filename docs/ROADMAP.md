...
Two directions discussed, not mutually exclusive, neither started:

- [ ] **A cap on the selectable date range** — the blunt, fast option. Straightforward to build (`RangePicker.tsx`'s drag handlers already clamp slot indices; add a max span there) but a real product tradeoff: it caps a real, useful feature (reviewing a full history's coverage at once) for a performance reason a user has no way to see coming, and doesn't help the first person to hit whatever the cap ends up being either — it only hides the problem at a smaller scale, not fixes it.
...
- [ ] Whichever direction (or both) gets picked, re-measure against the Demo Customer account's real 611-activity history rather than synthetic fixtures — it's already sitting there as a large, realistic worst case.
