...
- [x] **Stop scoping Fog/Heatmap to the date range at all** — done: Fog always takes the cheap precomputed-aggregate path, and Heatmap always takes the filtered path with one fixed, server-computed rolling window (never a client-supplied range), which is inherently cheap regardless of account age. The trigger for this item (an arbitrary, user-stretched range) no longer exists.
- [x] **A cap on the selectable date range** and **cheaper compositing for a wide filtered range** — both moot now that the item above removed the trigger; neither is needed.
