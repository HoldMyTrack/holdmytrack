...
Fog and Heatmap no longer take a date range or any other Activities-panel filter at all (see the design-decision item above) — both serve a plain precomputed lookup off `fog_tiles`, with no per-request compositing of any kind. Fog's own aggregate always covers every non-superseded activity; Heatmap's is rebuilt from only the activities inside a rolling window (`fog.HeatmapWindowDays`, 365 days), kept current by the same ingest/delete dirty-marking Fog already had, plus a weekly sweep (`internal/worker`'s `refreshHeatmapWindows`) so the window's trailing edge keeps moving even without new uploads.
