...
- [ ] Decide whether Fog of War and Heatmap should always show all-time coverage (DISTANCE/TYPE/hidden-track filters still apply, only the date range stops reaching them), rather than following the currently-selected date range the way Normal mode and the Activities list do.
- [ ] If so, the date-range control needs an honest UI answer for what it means while in Fog/Heatmap mode — greyed out with an explanation, or the mode switch visibly decoupling it — rather than silently doing nothing.
- [ ] Directly resolves the performance item below as a side effect, not a coincidence: if Fog/Heatmap never receive a date range, they never take the expensive filtered-compositing path for that reason (DISTANCE/TYPE/exclude filters could still trigger it, just not the common "I stretched the picker" case that prompted that item).

### Fog of War/Heatmap performance for a wide, non-default date range — a live compositing cost that scales with activity count, not a fixed budget

Only the *unfiltered* default view (`from`/`to`/`exclude` all absent) uses the cheap precomputed aggregate (`internal/fog/filtered.go`'s own comment: "the cheap precomputed fog_tiles aggregate"). The moment a date range is set — which is exactly what dragging the range-picker band to anything other than FR-6.1's default produces — every Fog of War/Heatmap tile request instead composites on the fly from every matching activity's raw per-activity z14 masks, then downsamples to the requested zoom. That cost scales directly with how many activities are in the selected range, not with anything bounded. Measured live: stretching the Demo Customer account's range-picker band to cover its full ~7-month, 611-activity history, then switching to Heatmap over the Dog Walks region, took ~15 seconds to render. `filteredCacheTTL` reuses an already-composited tile for a while, so a *repeat* request for the same range/viewport/zoom is cheap — the cost lands on the first person to ever ask for a given wide range, every time it changes.

Three directions discussed, not mutually exclusive, none started:

- [ ] **Stop scoping Fog/Heatmap to the date range at all** — the item directly above. If the date range never reaches these two modes, the common trigger for this cost (stretching the picker) stops existing; worth deciding that question first, since it may make the other two options moot or much lower priority.
- [ ] **A cap on the selectable date range** — the blunt, fast option, if the item above isn't adopted (or DISTANCE/TYPE/exclude filtering alone still proves expensive). Straightforward to build (`RangePicker.tsx`'s drag handlers already clamp slot indices; add a max span there) but a real product tradeoff: it caps a real, useful feature for a performance reason a user has no way to see coming, and doesn't help the first person to hit whatever the cap ends up being either.
- [ ] **Cheaper compositing for a wide filtered range** — the harder, more scalable option: precomputed rollups coarser than one-mask-per-activity (e.g. per-month or per-quarter aggregates) that a wide filtered query could composite from directly, the same shape `fog_tiles`' own all-time aggregate already takes for the unfiltered case, rather than always falling all the way back to raw per-activity masks. Needs its own design pass (how coarse, how they're kept up to date as new activities land, how `exclude` interacts with a precomputed rollup) before estimating the size of this.
- [ ] Whichever direction (or combination) gets picked, re-measure against the Demo Customer account's real 611-activity history rather than synthetic fixtures — it's already sitting there as a large, realistic worst case.
...
- [ ] Re-render affected tiles once fixed — existing `activity_tile_masks`/`fog_tiles` rows for already-ingested activities won't self-correct; this needs either a full re-seed (cheap for the Demo Customer account, per how the trim fix above was verified) or a proper backfill job for real accounts' existing history.

### Heatmap's saturation point is a hardcoded constant tuned on nothing — should be adaptive per account instead

`internal/fog/raster.go`'s `heatmapCap = 8.0` is the number of accumulated activity-passes through a pixel that maps to fully saturated (hottest yellow); the constant's own comment already admits it's "a first guess... explicitly open to retuning once there's real usage data" — there wasn't any until the Demo Customer account existed. There now is, and it argues the fixed value is wrong in a specific, visible way: **591 of the Demo Customer's 611 activities touch the home tile**, so at cap 8 that area was fully saturated after roughly the first two weeks of a 7-month account — the remaining ~98.6% of the history adds no further visual information there. The single most-used spot in the account looks like a flat blob, which is the opposite of what a heatmap should show.

- [ ] Replace the fixed `heatmapCap` with a per-account value computed from that account's own coverage, so a new, sparse account and a long-running, dense one both get a heatmap with real texture instead of one under- and the other over-saturating against the same constant.
- [ ] **Not live/per-request** — computing this on every tile view would add real cost to a path already flagged as expensive (the performance item above). Compute it once per account whenever `render_fog` already rebuilds `fog_tiles`/`activity_tile_masks` for other reasons, and store the result (e.g. a per-user column) to be read, not recomputed, at render time.
- [ ] **Median is probably the wrong statistic** — per-tile visit counts are likely heavily right-skewed for any real account (the Demo Customer's own data confirms the shape: 591/611 activities touch the home tile, while a one-off trip tile like Chicago or Niagara is touched by exactly 1), and the median of a distribution shaped like that lands close to the low end — using it directly as the cap would likely reproduce the same fast-saturation problem at a smaller number. A high percentile (90th/95th) of per-tile touch-counts among tiles with any coverage, or median × some multiplier, anchors "fully hot" near the genuinely busiest places instead of the typical ones — needs an actual tuning pass against real data (the Demo Customer account, and eventually real usage) rather than picking a second guess to replace the first.
- [ ] Compute over **tiles**, not pixels — a `GROUP BY` over `activity_tile_masks` per user is cheap; per-pixel intensity statistics across a whole account's coverage would be far more expensive for marginal extra accuracy.
