...
HoldMyTrack is a free, community-funded platform for tracking outdoor activities and seeing the accumulated shape of where you have been. It aggregates activity history the user already has — from watches, from cloud services, from files — and turns it into maps and exploration stats worth looking at. Three things it deliberately is **not**: it is not a fitness tracker (it begins where the watch's recording ends; the one exception, the Android app's plain GPS-only recording for a casual walk or drive, captures no sensor data or training metrics and goes through the same ingest pipeline as everything else), it has no social graph yet (no feed, follows, kudos, segments or leaderboards; see `docs/VISION.md` §5.7–§5.8 for why that ordering is deliberate rather than merely cautious), and it is not a health or fitness advisor (no HR zones, no training load, no recovery/readiness, no sleep tracking — pace and heart rate are shown per activity as route context, not analysed as a coaching product). It is funded by recurring community donations with public accounting, so there is no subscription tier to design around and no paywalled feature to hide behind.
