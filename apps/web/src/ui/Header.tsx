...
/**
 * Docked top chrome: the brand mark, then Donate, the upload control and the account menu at
 ...
 *
 * Upload and Export are both real and both passed in as elements rather than constructed
 * here, for the same reason: their wiring needs the map instance, which lives in MapView, not
 * here — keeping them props stops this component from needing to know anything about uploads
 ...
 *
 * The order is deliberate: Upload sits nearest Export because both are the two things this
...
export interface HeaderProps {
  ...
  /** Rendered between the upload control and the account menu. */
...
export function Header({ uploadControl, exportControl, onBrandClick, onOpenProfile, onOpenSettings }: HeaderProps) {
  const brand = (
    <>
