...
- **No final visual design.** No icon set, and the palette, type scale and fonts are the web's current ones carried over (Material 3) rather than a frozen design, and its screens wait on root `docs/ROADMAP.md` Phase 3's design freeze (`apps/android/docs/ROADMAP.md` Phase 5). The app's own chrome is light only; only the map follows the system dark setting.
