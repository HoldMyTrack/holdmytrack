...
| **Last updated** | 2026-09-24 (§7: recording is a one-tap button on the map with notification controls; synced recordings leave the device) |
...
4. **Long-press while recording or paused stops**, with haptic feedback. A long-press while idle does nothing.
...
6. **A notification is shown for as long as a recording is in progress**: "Recording" with a running elapsed-time counter, or "Paused · h:mm:ss"; below that, distance · current speed · current altitude, updated on every GPS fix. It carries **Pause**/**Resume** and **Stop** buttons that act exactly like the map button, and tapping it opens the map. Confirmed on an emulator, including Stop from the notification.
7. **Stop** saves the recording on the device, tagged not-synced, with no name or description and the activity type most recently set in Edit on this account (FR-5.2 step 4) — `"unknown"` for an account's first recording — and shows "Saved. Find it under Recorded Activities to queue it for sync." No network request happens at this step.
