...
**Notes — filtering**: Normal's tracks are narrowed to the selected date range (FR-2.6), sent as the tracks tile's `from`/`to`. Fog and Heatmap are never filtered, as on the web (`docs/SPEC.md` FR-4.2, FR-4.3). There is no TYPE/DISTANCE filter or per-track hide/show: on the web those live in the Activities panel, over its list, which this app doesn't have — a scope gap relative to the web client, not a bug.
...
**Behavior**:
1. Shown in Normal mode only, once the session is verified and the account has at least one activity day. Hidden in Fog and Heatmap (which ignore the range, as on the web) and while recording. The record button sits above it; MapLibre's logo and attribution move up above it while it's shown.
...
4. The range is applied when a knob or a held button is released: Normal's tracks are redrawn for it, and the camera flies to fit the range's drawn activities (`docs/SPEC.md` FR-6.6). Only a range the user picked moves the camera — not the default re-deriving itself, and not while recording.
...
2. Each row reads `"{type} · {distance} km · {status}"` (e.g. "walking · 0.23 km · Queued"), with a checkbox, a small preview of the route's shape (the recorded line alone in the map's track colour, no basemap, drawn from the points stored on the device), and Edit and Delete as icon buttons (a pencil and a red bin, labelled "Edit" and "Delete" for screen readers).
...
6. **Sync Now** (the existing Health Connect button, FR-3.2) also drains every queued row in the same tap — `flushRecordedQueue()`, run after the Health Connect pass or after it throws, submitting each as its own `POST /v1/sync/activities` call. **A row that syncs is deleted from the device** and disappears from this list; the activity now lives on the server, on the map and in the activity list. A row that fails stays queued for the next Sync Now. Confirmed on an emulator end to end: a queued walking recording became a `walking` activity server-side and the local row was gone.
...
8. A demo session can record and manage rows here, but every sync checkbox is disabled with an explanatory notice — queuing something "Sync Now" can never actually take would be pointless (matches FR-3's own demo gate).
