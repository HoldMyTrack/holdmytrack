...
- **`TRACK_COLOR = "#e0562f"` is called out in its own comment as not a token to fold into a future design system** — it is the color `apps/web/src/map/tracks.ts` paints tracks with, and belongs to the shared map style's visual identity across clients, not to this app's own theming.
