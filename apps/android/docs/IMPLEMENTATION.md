...
`activity_type` starts as `RecordingTypes.DEFAULT` (`"unknown"`) and nothing forces it away from that before Stop — a real gap found live: recording a walk with GPS Logger alongside Health Connect sync (both running on the same phone for the same walk, `source = "recorded"` vs `"healthconnect"`) produced two separate activities instead of one, because `docs/IMPLEMENTATION.md` §4.6's matcher requires an *exact* `activity_type` match and an unedited recording's `"unknown"` never equals whatever the same walk reported through Health Connect (typically a real type like `"walking"`).

Fixed client-side rather than by loosening the server's matcher, so "same type" stays an exact, reliable signal for every source rather than a fuzzy one only some rows benefit from: `RecordedActivitiesActivity`'s sync checkbox is disabled for any row still on `RecordingTypes.DEFAULT` (row subtitle explains why — "Set a type to sync"), so a row can only be queued once it carries a real type, picked from `RecordingTypes.PRESETS` or typed freely. `RecordingActivity.onSaveEdit` closes the one way around that gate: editing an already-queued row's type back to blank (Edit stays unlocked until sync, not just until Stop) demotes it back to not-synced rather than leaving a now-untyped row still queued. Confirmed on an emulator, not just read from source: the gate, the re-enable-on-retype, and the demote-on-re-blank all reproduced live (`apps/android/docs/SPEC.md` FR-5.2).

### 7.6 Verified on device
