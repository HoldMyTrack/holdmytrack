...
> **Scope note.** This app is Path 2 of `docs/adr/0001-three-independent-ingest-paths.md` — an on-device ingest client reading Health Connect — plus a full map viewer against the same accounts and the same style document the web client uses, plus casual, GPS-only in-app recording (`docs/VISION.md` §1.1, §4.1; `apps/android/docs/ROADMAP.md` Phase 7) — a deliberately separate capability from Health Connect sync, submitting a finished recording directly to the server rather than through Health Connect ([ADR-0007](../../../docs/adr/0007-in-app-gps-recording-submits-directly.md)). Everything in this document below describes that, as built and verified — Health Connect sync and everything before it on a physical device, in-app recording (Phase 7) on an emulator (`apps/android/docs/SPEC.md` §9 has the distinction, where it matters).

---

## 1. Architecture Overview

The Android app is a **single Gradle module against the platform SDK directly** — no architecture framework (no MVVM, no Compose, no dependency-injection library), no networking framework (`OkHttp` calls built by hand, no Retrofit), and no ORM (Phase 7's one local database is a plain `SQLiteOpenHelper`, not Room — `apps/android/docs/IMPLEMENTATION.md` §7.2). Seven Activities, one foreground `Service` (`recording/RecordingService`, Phase 7), twelve Kotlin files outside them, one `OkHttpClient` shared with the map SDK. This is a deliberate match to the app's actual size: a handful of screens and a handful of server calls do not carry the weight of a framework, and every dependency not taken is a version not to track and a surface not to secure.

**The app has a local write path now, not none.** Before Phase 7, everything it showed — the map, the tracks, the sync history — was a read against the same server the web client reads, and the one thing it *sent* (Health Connect sync batches) was normalized on-device and handed directly to the same ingestion pipeline every other path runs through (`docs/IMPLEMENTATION.md` §4.0–§4.1), with nothing kept locally beyond the two `SharedPreferences` files below. Phase 7 added a genuine local write: `recording/db/RecordedActivityStore` (`apps/android/docs/IMPLEMENTATION.md` §7.2) persists a recording — including its buffered points — the moment it stops, and is the first state this app holds that the server doesn't already know about. It stays a client, not a second source of truth: a row is deleted from the device the moment it syncs, so the store only ever holds what the server doesn't have yet (`apps/android/docs/SPEC.md` FR-5.2).

**The sync path and the display path are two disjoint, one-way pipes that never intersect on-device.** `sync/SyncRunner.kt` reads Health Connect and posts to `POST /v1/sync/activities`; `map/MapOverlays.kt` reads map tiles from `GET /tiles/v1/{tracks,fog,heatmap}` — both always against `BuildConfig.API_BASE_URL`, i.e. the HoldMyTrack server, never Health Connect. There is no code path anywhere in this app that renders a Health Connect record directly onto the map, and no code path in the sync engine that reads a tile. Once an activity has been synced, the app has no memory of it having come from Health Connect at all — the map renders the account's server-side history exactly as it would for a file upload or a cloud-provider push, and would keep working identically if Health Connect were removed entirely and every activity arrived by upload instead. The one line drawn from on-device data is a GPS recording while it's in progress (`MapOverlays.updateLiveTrack`, fed by `RecordingService`), and it's gone from the map the moment the recording stops — a stopped recording appears on the map only once it has synced and come back as server tiles like any other activity.
...
| `holdmytrack/` is its own Gradle build, not a module beside `poc-healthconnect/` | `poc-healthconnect/` is a throwaway Phase 1 diagnostic, deleted once its findings are recorded (`apps/android/docs/ROADMAP.md`); the real app was never meant to carry it |
| Host toolchain, not containerized | A container buys nothing an Android SDK on the host doesn't already have (`apps/android/README.md`). An AVD (`holdmytrack`, a Pixel 7 image) was added for Phase 7's emulator-based verification and does run Health Connect's own permission UI end to end, but FR-1 through FR-4's physical-device verification stands: background-read `ConsentRequired` behavior specifically (`docs/adr` and Phase 1's own findings) was measured on real hardware, not re-verified on the emulator |
...
graph TD
    ...
    App -->|Session/HoldMyTrackApi.kt| Auth
...
* **Local persistence**: two `SharedPreferences` files, both `MODE_PRIVATE`, no schema migration story because there is no schema — `holdmytrack.session` (the bearer token and email) and `holdmytrack.sync` (the per-account watermark, §1.1). Nothing else is persisted on-device.
