...
| :-- | :-- |
| `serve` | The HTTP API: ingest and sync endpoints, tile and raster routes, auth, the served style document. |
| `work` | The job worker: parsing, fog raster generation, tile precomputation. (Map export renders in the browser, not here — `docs/adr/0005-client-side-export-rendering.md`.) Dequeues from Postgres with `FOR UPDATE SKIP LOCKED` (§3.8, §4.1) — there is no broker, deliberately. |
